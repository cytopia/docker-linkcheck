...
_test-version:
	...
	@if [ "$(VERSION)" = "latest" ]; then \
		...
		LATEST="$$( \
			curl -Ss https://github.com/cytopia/linkcheck/tags \
			| tac \
			| tac \
		...
		if ! docker run --rm --platform $(ARCH) $(IMAGE):$(DOCKER_TAG) --version | grep -E "^linkcheck v$${LATEST}"; then \
		...
		if ! docker run --rm --platform $(ARCH) $(IMAGE):$(DOCKER_TAG) --version | grep -E "^linkcheck v$(VERSION)"; then \
			docker run --rm --platform $(ARCH) $(IMAGE):$(DOCKER_TAG) --version; \
			echo "Failed"; \
			exit 1; \
		fi; \
